*! texsave 1.4.2 03oct2018 by Julian Reif
* 1.4.2: added no tabularx option
...
program define texsave, nclass
	...
	* Default is to have first column left-justified and the rest centered.
	if `"`align'"'=="" {
		local align "l"
		local col_type = "C" // tabularx
		if "`tabularx'"!="" local col_type = "c" // tabular
		forval x = 2/`num_vars' {
			local align "`align'`col_type'"
		}
	}
	...
	* Headlines option
	...
	if "`tabularx'"=="" file write `fh' "\begin{tabularx}{`width'}{`align'}" _n(2)
	else file write `fh' "\begin{tabular}{`align'}" _n(2)
	...
	* Footnote style #1 only done if user specifies width option: this aligns with columns and needs to go before \end{tabular/x}
	...
	if "`tabularx'"=="" file write `fh' "\end{tabularx}" _n
	else file write `fh' "\end{tabular}" _n
